    ...
    if (directory != nullptr && directory->directory())
    {
        ...
        while (child != nullptr)
        {
            ...
            else if (child->name().ends_with(".lua"))
            {
                ...
                luaL_dostring(state.luaState, std::format("require('{}')", moduleName).c_str());
            }
            child = child->sibling();
        }
    }
...
static int searchModule(lua_State* state)
{
    ...
    const char* scriptsPath = lua_tostring(state, -1);
    std::string filePath = std::format("{}/{}", scriptsPath, module);
