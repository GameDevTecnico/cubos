...
TEST_CASE("ecs::Blueprint")
{
    ...
    SUBCASE("merge one blueprint into another blueprint and then spawn it")
    {
        ...
        blueprint.instantiate([&](std::string name) -> Entity { return merged.create("#sub" + name); },
                              [&](Entity entity, AnyValue component) { merged.add(entity, component); },
                              [&](Entity fromEntity, Entity toEntity, AnyValue relation) {
                                  merged.relate(fromEntity, toEntity, relation);
                              });
