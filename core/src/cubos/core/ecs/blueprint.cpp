...
bool Blueprint::addFromDeserializer(Entity entity, const std::string& name, old::data::Deserializer& deserializer)
{
...
void Blueprint::merge(const std::string& prefix, const Blueprint& other)
{
    ...
    old::data::SerializationMap<Entity, std::string> srcMap;
    ...
    old::data::Context src;
    old::data::Context dst;
    src.push(srcMap);
    dst.push(mMap);
