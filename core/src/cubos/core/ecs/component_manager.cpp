...
old::data::Package ComponentManager::pack(uint32_t id, std::size_t componentId, old::data::Context* context) const
{
    return mEntries[componentId - 1].storage->pack(id, context);
}

bool ComponentManager::unpack(uint32_t id, std::size_t componentId, const old::data::Package& package,
                              old::data::Context* context)
{
