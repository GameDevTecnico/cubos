...
static bool parse(fs::path path, std::vector<Component>& components)
{
    ...
    else if (path.extension() == ".hpp")
    {
        ...
        while (!parser.eof())
        {
            ...
            if (parser.acceptKeyword("#include"))
            {
                if (parser.acceptKeywords({"<components/base.hpp>", "\"components/base.hpp\""}))
                    included = true;
                else
                    parser.skipToken();
            }
            ...
            else if (parser.acceptKeyword("struct"))
            {
                ...
                if (!included)
                {
                    parser.error("Expected '#include <components/base.hpp>' before component declaration");
                    return false;
                }
...
static bool generate(const GenerateOptions& options)
{
    ...
    for (auto& component : components)
    {
        ...
        file << std::endl;
        file << "#define CUBINHOS_GENERATE" << std::endl;
        file << "#include " << fs::absolute(component.header) << std::endl;
        file << "#undef CUBINHOS_GENERATE" << std::endl;
        file << std::endl;
