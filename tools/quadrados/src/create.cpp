...
struct State
{
    ...
    std::vector<fs::path> templates; ///< Available templates.
};
...
// TODO: comments
void getTemplates(const GlobalArgs& ga, State& state)
{
    (void)state;
    ...
    // TODO: remove (debug)
    std::cerr << "templatesPath: " << templatesPath << std::endl;
    ...
    // TODO: remove (debug)
    for (const auto& temp : templatesPath)
    {
...
// TODO: comments
// TODO: change to getTemplate(extension, state)
bool templateExists(const std::string& extension, const State& state)
{
    (void)extension, (void)state;
...
static bool generate(const CreateOptions& options, const GlobalArgs& ga)
{
    ...
    for (const auto& asset : options.assets)
    {
        ...
        // TODO:
        // const auto extension = asset.extension().string();
