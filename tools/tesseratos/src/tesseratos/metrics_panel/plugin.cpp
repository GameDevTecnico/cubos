...
static void showMetricsSystem(const DeltaTime& deltaTime, Metrics& metrics)
{
    ...
    for (std::size_t i = 0; i < metrics.timeHistory.size(); ++i)
    {
        ...
        if (metrics.timeHistory[i] <= metrics.timeSample)
        {
            ...
            metrics.fpsHistory.erase(metrics.fpsHistory.begin() + static_cast<std::vector<float>::difference_type>(i));
        }
    }
