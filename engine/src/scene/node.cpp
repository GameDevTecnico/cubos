...
void SceneNode::add(const Type& type, nlohmann::json component)
{
...
void SceneNode::relateFrom(const Type& type, std::string from, nlohmann::json data)
{
    ...
    mRelationsFrom.at(type).insert_or_assign(from, data);
}

void SceneNode::relateTo(const Type& type, std::string to, nlohmann::json data)
{
    ...
    mRelationsTo.at(type).insert_or_assign(to, data);
}

void SceneNode::unrelateFrom(const Type& type, std::string from)
{
    if (mRelationsFrom.contains(type))
    {
        mRelationsFrom.at(type).erase(from);
    }
}

void SceneNode::unrelateTo(const Type& type, std::string to)
{
    if (mRelationsTo.contains(type))
    {
        mRelationsTo.at(type).erase(to);
    }
}

const std::unordered_map<std::string, SceneNode>& SceneNode::children() const
{
    return mChildren;
}
...
SceneNode& SceneNode::create(const std::string& name)
{
    ...
    {
        mChildren.emplace(name, SceneNode{});
    }
