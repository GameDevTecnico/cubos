...
Entity Scene::loadEntities(const SceneNode& node, const Assets& assets, std::string name)
{
    ...
    if (!node.inherits().isNull())
    {
        assets.read(node.inherits())
            ->blueprint()
            .instantiate(
                [&](std::string inheritsName) -> Entity {
                ...
                [&](Entity entity, AnyValue component) { mBlueprint.add(entity, component); },
                [&](Entity fromEntity, Entity toEntity, AnyValue relation) {
                    mBlueprint.relate(fromEntity, toEntity, relation);
                });
    ...
    {
        CUBOS_INFO("LOADING CHILD {} OF {}", childName, name);
        Entity childEntity = this->loadEntities(childNode, assets, name + "#" + childName);
...
bool Scene::loadComponentsAndRelations(const SceneNode& node, std::string name)
{
    ...
    for (const auto& [childName, childNode] : node.children())
    {
        if (!this->loadComponentsAndRelations(childNode, name + "#" + childName))
        {
            return false;
        }
    }
