...
void Assets::cleanup()
{
    ...
    for (const auto& entry : mEntries)
    {
        ...
        if (entry.second->status == Status::Loaded && entry.second->refCount == 0)
        {
            ...
            CUBOS_DEBUG("Unloaded asset {}", core::old::data::Debug(AnyAsset(entry.first)));
        }
    }
}
...
void Assets::loadMeta(std::string_view path)
{
    ...
    else if (file->name().ends_with(".meta"))
    {
        ...
        auto meta = AssetMeta();
        auto des = core::old::data::JSONDeserializer(contents);
        des.read(meta);
        if (des.failed())
        {
        ...
        CUBOS_DEBUG("Loaded asset {} metadata from '{}'", core::old::data::Debug(handle), path);
    }
}
...
AnyAsset Assets::load(AnyAsset handle) const
{
    ...
    if (assetEntry->status != Assets::Status::Loaded)
    {
        ...
        {
            CUBOS_TRACE("Queuing asset {} for loading", core::old::data::Debug(handle));
...
bool Assets::saveMeta(const AnyAsset& handle) const
{
    ...
    {
        CUBOS_DEBUG("Saving metadata for asset {}", core::old::data::Debug(handle));
        ...
        auto serializer = core::old::data::JSONSerializer(*stream, 4);
...
bool Assets::save(const AnyAsset& handle) const
{
    ...
    CUBOS_DEBUG("Saving data for asset {}", core::old::data::Debug(handle));
...
void Assets::invalidate(const AnyAsset& handle, bool shouldLock)
{
    ...
    if (assetEntry->data != nullptr)
    {
        ...
        CUBOS_DEBUG("Invalidated asset {}", core::old::data::Debug(handle));
    }
}
...
AnyAsset Assets::store(AnyAsset handle, std::type_index type, void* data, void (*destructor)(void*))
{
    ...
    CUBOS_DEBUG("Stored data of type {} for asset {}", type.name(), core::old::data::Debug(handle));
...
void* Assets::access(const AnyAsset& handle, std::type_index type, Lock& lock, bool incVersion) const
{
    ...
    {
        CUBOS_DEBUG("Loading asset {} as a dependency", core::old::data::Debug(handle));
        ...
        {
            CUBOS_CRITICAL("Could not load asset {}", core::old::data::Debug(handle));
            abort();
        }
        lock.lock();
    }
    ...
    {
        assetEntry->version++;
        CUBOS_DEBUG("Incremented version of asset {}", core::old::data::Debug(handle));
    }
...
std::shared_ptr<Assets::Entry> Assets::entry(const AnyAsset& handle) const
{
    ...
    if (it == mEntries.end())
    {
        CUBOS_ERROR("No such asset {}", core::old::data::Debug(handle));
        return nullptr;
    }
...
std::shared_ptr<Assets::Entry> Assets::entry(const AnyAsset& handle, bool create)
{
    ...
    if (it == mEntries.end())
    {
        ...
        if (create)
        {
            ...
            CUBOS_TRACE("Created new asset entry for {}", core::old::data::Debug(handle));
        }
        else
        {
            CUBOS_ERROR("No such asset {}", core::old::data::Debug(handle));
...
std::shared_ptr<AssetBridge> Assets::bridge(const AnyAsset& handle) const
{
    ...
    CUBOS_ERROR("Cannot find a bridge for asset {}: asset does not have a path", core::old::data::Debug(handle));
    return nullptr;
}
...
void Assets::loader()
{
    for (;;)
    {
        ...
        {
            CUBOS_ERROR("Failed to load asset '{}'", core::old::data::Debug(task.handle));
